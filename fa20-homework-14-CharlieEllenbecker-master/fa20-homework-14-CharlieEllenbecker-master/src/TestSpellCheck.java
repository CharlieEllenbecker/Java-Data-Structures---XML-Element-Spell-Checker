...
public class TestSpellCheck extends LockedTestCase {
	...
	public void test0() {
		...
		assertEquals(Ti(1398740451),bad.size());
	}
	
	public void test1() {
		bad = sc.check(new Element("p","One"));
		assertEquals(Ti(1462559208),bad.size());
	}
	
	public void test2() {
		bad = sc.check(new Element("p","Ooone"));
		assertEquals(Ti(1316343018),bad.size());
		assertEquals("Ooone",bad.get(0));
	}
	
	public void test3() {
		bad = sc.check(new Element("p","Go home"));
		assertEquals(Ti(625024015),bad.size());
	}
	
	public void test4() {
		bad = sc.check(new Element("p"," Go home. "));
		assertEquals(Ti(1208487756),bad.size());
	}
	
	public void test5() {
		bad = sc.check(new Element("h1","Goooo west, young man."));
		assertEquals(Ti(2016254634),bad.size());
		assertEquals("Goooo",bad.get(0));
	}
	
	public void test6() {
		bad = sc.check(new Element("p","Vladimir Putin called Barack Obama"));
		assertEquals(Ti(318826190),bad.size());
	}
	
	public void test7() {
		bad = sc.check(new Element("script","js.inc(tv);"));
		assertEquals(Ti(53062151),bad.size());
	}
	
	public void test8() {
		bad = sc.check(new Element("style","head.font = RED;", " docfont = ITALIC"));
		assertEquals(Ti(1427375200),bad.size());
	}
	...
	public void test9() {
		bad = sc.check(new Element("html",
				...
				new Element("body",
						...
						new Element("p","tumtum tree: horticulture."))));
		assertEquals(Ti(1520996045),bad.size());
		assertEquals("Tyypo",bad.get(0));
		assertEquals("tumtum",bad.get(1));
	}
	...
	Element e19 = new Element("font",Collections.singletonMap("size","+1"),"temporarilly");
	Element e20 = new Element("b","increase");
	Element e21 = new Element("font",Collections.singletonMap("size","5"),"veeery small.");
	Element e16 = new Element("font",Collections.singletonMap("size","-1"),"Sometimes however, the xfont becomes ",e17,"although ",e18,"words can occur and\nwe can ",e19,e20,"the size or make it\n",e21);
