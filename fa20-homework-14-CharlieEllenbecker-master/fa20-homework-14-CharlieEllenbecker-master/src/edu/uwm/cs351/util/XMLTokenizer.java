...
import static edu.uwm.cs351.util.XMLTokenType.ATTR;
import static edu.uwm.cs351.util.XMLTokenType.CLOSE;
import static edu.uwm.cs351.util.XMLTokenType.ECLOSE;
import static edu.uwm.cs351.util.XMLTokenType.END;
import static edu.uwm.cs351.util.XMLTokenType.ERROR;
import static edu.uwm.cs351.util.XMLTokenType.ETAG;
import static edu.uwm.cs351.util.XMLTokenType.OPEN;
import static edu.uwm.cs351.util.XMLTokenType.TEXT;
...
public class XMLTokenizer implements Iterator<XMLTokenType>, Iterable<XMLTokenType> {
	...
	/**
	 * Creates a new XMLTokenizer for the given reader.
	 * @param r the reader from which to generate tokens
	 */
	 ...
	 * @return the most recently returned token
	 */
	 ...
	 * or null otherwise.
	 * 
	 * @return tag name (if OPEN or ETAG), key (if ATTR), or null
	 */
	 ...
	 * or null otherwise.
	 * 
	 * @return value (if ATTR), text (if TEXT), error message (if ERROR), or null
	 */
	...
	public boolean hasNext() {
		...
		return currentToken != null && currentToken != END;
	}

	public XMLTokenType next() {
		if (!hasNext()) currentToken = END; // throw new NoSuchElementException("at end of stream");
		saved = false;
		return currentToken;
	}
	...
	/**
	 * Updates the line offset
   * @param s string to search for newlines
   */
  ...
  private class OuterState implements State {
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			test_tokenizer(false,new InputStreamReader(System.in));
		} else {
			boolean read_html = false;
			for (String arg : args) {
				if (arg.equals("--html")) read_html = true;
				else try {
					System.out.println("\n *************** " + arg + "\n");
					test_tokenizer(read_html,new FileReader(arg));
	...
	static void test_tokenizer(boolean isHTML, Reader r) {
